...
import (
	...
	"messengermax/auth/internal/service"
...
func main() {
	...
	if err != nil {
		log.Printf("auth: user-service unreachable at %s even after retries: %v", cfg.Services.UserAddr, err)
	} else {
...
// auth starts before user-service and silently skips seeding user profiles.
func dialWithRetry(ctx context.Context, addr string, totalTimeout, interval time.Duration) (*grpc.ClientConn, error) {
	...
	for {
		...
		log.Printf("auth: user-service at %s not ready: %v; retrying in %s", addr, err, interval)
		select {
		case <-ctx.Done():
			return nil, ctx.Err()
