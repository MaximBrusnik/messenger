...
func (g *Gateway) GetCallHistory(c *gin.Context) {
	if g.calls == nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": "сервис звонков недоступен"})
		return
	}
	uid := userID(c)
...
func (g *Gateway) GetCall(c *gin.Context) {
	...
	id, err := parseID(c, "id")
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "неверный ID звонка"})
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "звонок не найден"})
		return
	}
	if resp.Id != id {
		c.JSON(http.StatusNotFound, gin.H{"error": "звонок не найден"})
