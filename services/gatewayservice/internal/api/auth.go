...
func (g *Gateway) VerifyEmail(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"message": "Email успешно подтверждён", "token": resp.Token})
}
...
func (g *Gateway) ResendVerification(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"message": "Письмо отправлено повторно"})
}
...
func (g *Gateway) UpdateProfile(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"message": "Профиль обновлен", "data": userJSON(resp, true)})
}
