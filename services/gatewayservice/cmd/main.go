...
func main() {
	...
	v1 := router.Group("/api/v1")
	{
		...
		protected.Use(authMW.Required)
		{
			...
			admin.Use(func(c *gin.Context) {
				if !gw.IsAdmin(c) {
					c.AbortWithStatusJSON(http.StatusForbidden, gin.H{"error": "РЅРµС‚ РїСЂР°РІ Р°РґРјРёРЅРёСЃС‚СЂР°С‚РѕСЂР°"})
					return
				}
				c.Next()
			})
	...
	port := ":" + cfg.ServerPort
	log.Printf("API-gateway РЅР° РїРѕСЂС‚Сѓ %s (%s)", port, cfg.Env)
