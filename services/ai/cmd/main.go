...
import (
	...
	"messengermax/ai/internal/gemini"
...
func main() {
	...
	if err != nil {
		log.Fatal("ai: chat-service unreachable: ", err)
	}
	...
	consumer := nats.NewConsumer(cfg.NATS.URL, "ai-service", []string{nats.TopicAITrigger}, handle)
	go consumer.Run(ctx)
...
func handleTrigger(ctx context.Context, chatClient pbchat.ChatServiceClient, geminiClient *gemini.Client, value []byte) {
	...
	// retry a few times because messages may still be in flight to chat-service
