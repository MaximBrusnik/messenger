...
import (
	...
	"messengermax/realtime/internal/consumer"
	"messengermax/realtime/internal/hub"
	"messengermax/realtime/internal/service"
	"messengermax/realtime/internal/ws"
)

func main() {
	cfg := config.Load()
	cfg.ServiceName = "realtime"
	...
	if err != nil {
		log.Printf("realtime: user-service unreachable at %s: %v", cfg.Services.UserAddr, err)
	} else {
	...
	defer cancel()
	c1 := nats.NewConsumer(cfg.NATS.URL, "realtime-chat", []string{
	...
	c2 := nats.NewConsumer(cfg.NATS.URL, "realtime-users", []string{nats.TopicUserEvents}, func(t, k string, v []byte) {
