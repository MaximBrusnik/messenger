...
func NewController(jwtManager *jwt.Manager, h *hub.Hub, redis *redis.Client, producer *nats.Producer) *Controller {
...
func (c *Controller) Handle(gctx *gin.Context) {
	...
	c.hub.SendToUser(userID, hub.WSMessage{Type: "CONNECTED", Payload: gin.H{"user_id": userID}})
	c.hub.Broadcast(hub.WSMessage{Type: "USER_STATUS", Payload: gin.H{"user_id": userID, "status": "online"}})
...
func (c *Controller) DisconnectUser(ctx context.Context, userID uint) error {
	...
	c.hub.Broadcast(hub.WSMessage{Type: "USER_STATUS", Payload: gin.H{"user_id": userID, "status": "offline"}})
