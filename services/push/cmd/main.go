...
import (
	...
	"messengermax/push/internal/fcm"
	"messengermax/push/internal/repo"
	"messengermax/push/internal/service"
)

func main() {
	cfg := config.Load()
	cfg.ServiceName = "push"
	...
	consumer := nats.NewConsumer(cfg.NATS.URL, "push-service", []string{
	...
	case nats.TopicMessageCreated:
		...
		for _, pid := range e.Participants {
			...
			if !redisClient.IsOnline(ctx, uint(pid)) {
				sender.SendPush(ctx, uint(pid), "Новое сообщение", truncate(e.Message.Text, 80))
			}
		}
