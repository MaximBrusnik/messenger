...
func NewController(jwtManager *jwt.Manager, h *hub.Hub, manager CallStateManager) *Controller {
...
func (c *Controller) Handle(gctx *gin.Context) {
	...
	c.hub.SendToUser(uint(userID), hub.WSMessage{Type: "CALL_CONNECTED", Payload: gin.H{"user_id": userID}})
	go c.pingLoop(conn)
...
func (c *Controller) handleInvite(ctx context.Context, callerID uint, raw json.RawMessage) {
	...
	if err := json.Unmarshal(raw, &p); err != nil || p.CalleeID == 0 {
		c.hub.SendToUser(callerID, hub.WSMessage{Type: "CALL_ERROR", Payload: gin.H{"error": "invalid invite"}})
		return
	}
	...
	c.hub.SendToUser(callerID, hub.WSMessage{Type: "CALL_INVITE_ACK", Payload: callPayload(call)})
	...
	// Ring the callee.
	c.hub.SendToUser(uint(p.CalleeID), hub.WSMessage{
		Type: "CALL_RINGING",
		Payload: gin.H{
			"call_id":   call.ID,
			"caller_id": callerID,
...
func (c *Controller) handleAccept(ctx context.Context, userID uint, raw json.RawMessage) {
	...
	c.broadcastRoom(uint64(call.ID), hub.WSMessage{Type: "CALL_ACCEPTED", Payload: callPayload(call)})
}
...
func (c *Controller) handleReject(ctx context.Context, userID uint, raw json.RawMessage) {
	...
	c.broadcastRoom(uint64(call.ID), hub.WSMessage{Type: "CALL_REJECTED", Payload: callPayload(call)})
...
func (c *Controller) handleEnd(ctx context.Context, userID uint, raw json.RawMessage) {
	...
	c.broadcastRoom(uint64(call.ID), hub.WSMessage{Type: "CALL_ENDED", Payload: callPayload(call)})
	c.removeRoom(uint64(call.ID))
}
...
func (c *Controller) handleRelay(userID uint, raw json.RawMessage) {
	...
	c.hub.SendToUser(uint(target), hub.WSMessage{Type: full.Type, Payload: full.Payload})
}
...
func (c *Controller) failCall(ctx context.Context, call *entity.Call, reason string, callerID, calleeID uint) {
	...
	c.hub.SendToUser(callerID, hub.WSMessage{Type: "CALL_ENDED", Payload: callPayload(call)})
	c.removeRoom(uint64(call.ID))
}
...
func (c *Controller) broadcastRoom(callID uint64, msg hub.WSMessage) {
